...
demo = {
  ...
  APITable: function ordenarTabla() {
    // Obtener la tabla y las filas
    var tabla = document.getElementById("miTabla");
    var filas = tabla.rows;
    
    // Crear un array con los valores a ordenar
    var valores = [];
    for (var i = 1; i < filas.length; i++) { // Empezar en 1 para saltar la fila de encabezado
      valores.push(parseFloat(filas[i].cells[1].textContent)); // Suponiendo que el número está en la segunda celda
    }
    
    // Ordenar el array
    valores.sort(function(a, b){return a-b});
    
    // Actualizar la tabla con los nuevos valores ordenados
    for (var i = 0; i < valores.length; i++) {
      for (var j = 1; j < filas.length; j++) {
        if (parseFloat(filas[j].cells[1].textContent) === valores[i]) { // Suponiendo que el número está en la segunda celda
